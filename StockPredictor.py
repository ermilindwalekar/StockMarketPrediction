...
def ReadableDateRange(startDate, endDate, period, weekends = False):
    #The start and end date
    sd = datetime.strptime(startDate, '%Y-%m-%d')
...
def PlotData(df,name,p = None):
    ...
    #Set y-axis label
    mpl.ylabel(name+' High Value')
    #Add the label in the upper left
    mpl.legend(loc = 'upper left')
    mpl.savefig('BeforePrediction.png', bbox_inches='tight')
    #mpl.show()
...
class StockPredictor:
    ...
    def PredictDate(self, startDate, endDate, period = 'weekly'):
        ...
        Rts = ReadableDateRange(startDate, endDate, period)
        m = ts.shape[0]
        ...
        PRts = pd.DataFrame(data=None, index=None, columns=None, dtype=None, copy=False)
        ...
        PRts['Date'] = Rts
        #print(PRts)
        result = pd.concat([P, PRts], axis=1, join_axes=[P.index])
        #print(result)
        R = result.iloc[:,[7,0,1,2,3]]
        return P,R
